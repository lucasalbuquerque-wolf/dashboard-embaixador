...
export const FAQ = [
  ...
  },
  {
    ...
    items: [
      ...
      { q: 'Custo', a: 'O gasto real do programa no período = fixo pago aos embaixadores ATIVADOS (só quem tem valor de fixo; permuta/só-comissão não têm) + comissão paga (as 3 primeiras parcelas dos clientes indicados). O fixo conta a partir de jan/2026. Só o programa embaixador tem fixo — parceiro não.' },
  ...
  },
  {
    ...
    items: [
      ...
      { q: 'O que é LTV/CAC? (referência, não veredito)', a: 'Valor do cliente na vida ÷ custo de aquisição. ≥ 3 saudável, 1–3 marginal, < 1 destrói valor (origem do 3×: David Skok). AQUI é REFERÊNCIA, não veredito. O LTV usa a vida MÉDIA (Kaplan-Meier com extrapolação de cauda). MAS a distribuição é BIMODAL: boa parte dos clientes cancela no cliff dos meses 4-5 (logo após a comissão de 3 meses) e vive pouco; os sobreviventes ficam muito tempo e puxam a média pra cima, bem acima da mediana. Os números exatos (mediana, % em 6 e 12 meses, média) estão no card "Lifetime do cliente" na seção Churn & saúde. Como a média esconde esse risco, a decisão deve olhar o CAC Payback e o alerta do cliff na seção Retenção.' },
  ...
  },
  {
    cat: 'Outros cálculos',
    items: [
      { q: 'Break-even do programa', a: 'Curva acumulada desde jan/2026 de (receita − comissão − fixo). Quando cruza o zero, o programa se pagou. O fixo é contado por mês só para os embaixadores ATIVADOS com valor de fixo (a partir de jan/2026, esquecendo 2025); a comissão são as 3 primeiras parcelas de cada cliente. IMPORTANTE: com o custo fixo COMPLETO (antes 93% dele sumia dos cálculos), o acumulado do embaixador está NEGATIVO — o fixo mensal supera o MRR que ele gera. Não descontamos custo de servir/COGS: é receita cheia. Obs.: sob o filtro "parceiro" não há fixo (parceiro não tem), então a curva dele é só receita − comissão.' },
      ...
      { q: 'GRR / churn de receita', a: 'GRR (Gross Revenue Retention) = (MRR da base no início − MRR que cancelou dessa base) ÷ MRR no início. Mede quanto da receita da base você reteve, ignorando vendas novas. Mostramos o GRR MENSAL equivalente (normalizado pelo tamanho da janela, igual ao churn mensal) — comparável com o benchmark de mercado; bom acima de 85%. O GRR acumulado do período aparece no tooltip, mas depende do tamanho da janela. É mais importante que o churn por contagem numa base concentrada, onde perder 1 cliente grande ≠ perder 1 pequeno.' },
      { q: 'Lifetime do cliente', a: 'Vem da CURVA de sobrevivência (Kaplan-Meier): mostramos a MEDIANA (metade dos clientes cancela até aí), o % que chega a 6 e a 12 meses, e a MÉDIA (com extrapolação de cauda). A média é o que o LTV usa; ela é maior que a mediana porque a vida é bimodal — muitos cancelam cedo (cliff dos meses 4-5, fim da comissão) e os sobreviventes vivem muito. Não usamos mais o "1 ÷ churn" simples, que assume churn constante e superestima (o churn é front-loaded).' },
